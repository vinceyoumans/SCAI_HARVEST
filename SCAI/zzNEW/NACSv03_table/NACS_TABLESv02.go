...
//note - original Endpoint had one long Term Description
// that should have been multile lines.
// conversion chops the lines up into a slice of strings
...
func main() {
	...
	for i := 0; i <= len(NS)-1; i++ {
		_, err := client2.Collection("NONACSv01_table_v03").Doc(NS[i].Name).Set(context.Background(), NS[i])
		if err != nil {
			log.Fatal(err)
		}
	}

	jsonFile, err := os.Create("./NACS_TABLE.json")
